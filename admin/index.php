...
:root {
    ...
    /* Botón secundario - verde (sugerencia) */
    --btn-secundario-bg: linear-gradient(135deg, #2E7D32, #1B5E20);
    --btn-secundario-hover: linear-gradient(135deg, #388E3C, #2E7D32);
    
    /* Botón terciario - ámbar/dorado (sugerencia) */
    --btn-tercero-bg: linear-gradient(135deg, #F57C00, #E65100);
    --btn-tercero-hover: linear-gradient(135deg, #FB8C00, #F57C00);
    
    /* Sombras */
    --sombra-normal: 0 6px 18px rgba(0, 0, 0, 0.35);
    --sombra-hover: 0 10px 24px rgba(0, 0, 0, 0.45);
    --sombra-active: 0 4px 12px rgba(0, 0, 0, 0.4);
    
    /* Bordes y fondos */
    --bg-tabla: rgba(20, 40, 25, 0.45);
    --bg-th: rgba(27, 94, 32, 0.95);
    --bg-td: #ffffff;
    --bg-td-hover: #f5f5f5;
    --borde-blanco: rgba(255, 255, 255, 0.25);
    --borde-tabla: #e0e0e0;
    
    /* Colores de estado */
    --estado-inventario: #1565C0;
    --estado-proceso: #F9A825;
    ...
    --font-size-titulo: 44px;
    --font-size-bienvenida: 20px;
    --font-size-tabla: 15px;
    --font-size-tabla-celda: 15.5px;
}
...
.bienvenida {
    text-align: center;
    font-size: var(--font-size-bienvenida);
    margin-bottom: 20px;
}

.table-box {
    background: var(--bg-tabla);
...
td {
    ...
    padding: 20px 18px;
    line-height: 1.4;
    font-size: var(--font-size-tabla-celda);
...
.btn-tercero {
    background: var(--btn-tercero-bg);
    box-shadow: var(--sombra-normal);
}

.btn-tercero:hover {
    background: var(--btn-tercero-hover);
    transform: translateY(-2px);
    box-shadow: var(--sombra-hover);
}
...
<div class="navbar">
    ...
    <div class="nav-links">
        ...
        <a href="agregar_inventario.php">Inventario</a>
        <a href="buscar_codigo.php">Iniciar Proceso</a>
        <a href="registrar_produccion.php">Produccion</a>
        <a href="registro_despacho.php">Despacho</a>
...
<div class="barra-botones">
    <a href="paquetes.php" class="btn-tercero">📦 Ver Paquetes</a>
</div>
<div class="table-box">
<h2>Inventario</h2>
...
if ($r && $r->num_rows > 0) {
 ...
 <td>{$f['hora']}</td>
    ...
    <td class='col-acciones'>
        <a class='btn-accion btn-editar' href='editar_inventario.php?id={$f['id']}'>Editar</a>
        <a class='btn-accion btn-eliminar'
           href='eliminar_inventario.php?id={$f['id']}'
...
if ($proc->num_rows > 0) {
    while ($p = $proc->fetch_assoc()) {
        ...
        <td class='col-acciones'>
            <a class='btn-accion btn-editar' href='editar_proceso.php?id={$p['id']}'>Editar</a>
            <a class='btn-accion btn-eliminar'
               href='eliminar_proceso.php?id={$p['id']}'
...
if ($prod->num_rows > 0) {
    while ($p = $prod->fetch_assoc()) {
        echo "<tr>
            ...
            <td class='col-acciones'>
                <a class='btn-accion btn-editar' href='editar_produccion.php?id={$p['id']}'>Editar</a>
                <a class='btn-accion btn-eliminar' href='eliminar_produccion.php?id={$p['id']}' onclick=\"return confirm('¿Eliminar este registro de producción?')\">Eliminar</a>
            </td>
        </tr>";
    }
} else {
...
if ($despachos->num_rows > 0) {
    while ($d = $despachos->fetch_assoc()) {
        echo "<tr>
            ...
            <td class='col-acciones'>
 <a class='btn-accion btn-editar' href='editar_despacho.php?id={$d['id_despacho']}'>Editar</a>
<a class='btn-accion btn-eliminar' href='eliminar_despacho.php?id={$d['id_despacho']}'>Eliminar</a>
            </td>
        </tr>";
    }
} else {
